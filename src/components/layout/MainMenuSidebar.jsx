...
export function MainMenuSidebar({ active, mobileOpen = false, onClose, onNavigate }) {
  ...
  return (
    <aside
      ...
      className={`fixed inset-y-0 left-0 z-[60] flex w-72 shrink-0 flex-col border-r border-slate-200 bg-white p-4 shadow-xl transition-transform duration-200 lg:static lg:z-auto lg:w-64 lg:translate-x-0 lg:shadow-none ${mobileOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
    >
        ...
        <nav className="flex flex-col gap-1">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <span
                  className={`material-symbols-outlined ${isActive ? 'text-primary' : 'text-slate-500 group-hover:text-primary'
                    }`}
                >
                  {item.icon}
                </span>
